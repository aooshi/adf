...
namespace Adf.IO
{
    ...
    /// 字节写书器(Little Endian)
    /// </summary>
    public class BinaryWriterLE : IBinaryWriter, IDisposable
    {
        ...
        public BinaryWriterLE(Stream output)
        {
            this.stream = output;
            this.encoding = new UTF8Encoding();
        }

        public BinaryWriterLE(Stream output, Encoding encoding)
        {
            this.stream = output;
        ...
        {
            Adf.BaseDataConverter.ToBytesLE(value, _buffer, 0);
