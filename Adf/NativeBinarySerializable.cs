...
namespace Adf
{
    ...
    /// 本地数据二进制处理
    /// </summary>
    public sealed class NativeBinarySerializable : IBinarySerializable
    {
        ...
        readonly IBinarySerializable binarySerializable;
        ...
        /// <returns></returns>
        public bool IsHandled(object value)
        {
            if (value is bool ||
                value is byte ||
                value is string ||
        ...
        public byte[] Encode(object value)
        {
            ...
            //throw new NotSupportedException("not support encode type " + value.GetType().FullName);
        }

        public byte[] Encode(DateTime value)
        {
            byte[] bytes = ToBytes(value.Ticks);
        ...
        public byte[] Encode(string value)
        {
            ...
            byte[] asBytes = this.encoding.GetBytes(value);
        ...
        public byte[] Encode(StringBuilder value)
        {
            if (value == null)
        ...
        public DateTime DecodeDate(byte[] bytes)
        {
            return new DateTime(ToLong(bytes));
        }

        public bool DecodeBool(byte[] bytes)
        {
            if (bytes == null)
        ...
        public Int32 DecodeInteger(byte[] bytes)
        {
            return ToInt(bytes);
        }
        ...
        public string DecodeString(byte[] bytes)
        {
            ...
            return this.encoding.GetString(bytes, 1, bytes.Length - 1);
        }

        public char DecodeCharacter(byte[] bytes)
        {
        ...
        public StringBuilder DecodeStringBuilder(byte[] bytes)
        {
